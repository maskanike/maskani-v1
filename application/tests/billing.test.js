import {
  aUserExistsWith, aFlatExistsWith, aUnitExistsWith, aTenantExistsWith, truncateModel, getAllFromModel,
} from './utils.test';
...
describe('Billing', () => {
  ...
  describe('/billing/invoice', () => {
    it('send an invoice to tenant and add record to the database', async () => {
      ...
      assert.equal(dbStatement[0].amount, (invoice.rent + invoice.garbage + invoice.penalty + invoice.water));
    });
    it.skip('should get a list of all invoices in the database', async () => { });
  });
  ...
  describe('/billing/receipts', () => {
    ...
    it.skip('should get a list of all receipts in the database', async () => { });
  });
});
